...
def register(ctx):
    try:
        from . import analysis_tools, commands, food, query
    except ImportError:
        logger.warning("health-data plugin failed to import", exc_info=True)
    ...
    _register_tool(
        ...
        description=(
            ...
            "health-coach skill: cite dates/numbers, compare to the user's "
            "baseline, and frame cross-domain links as hypotheses."
        ),
    )
    _register_tool(
        ctx,
    ...
    _register_tool(
        ...
        SYNC_SCHEMA,
        description=(
            "Sync health data. For any health question, follow the "
            "health-coach skill before answering."
        ),
    )
    _register_tool(
        ctx,
        ...
        CALENDAR_PEEK_SCHEMA,
        description="Peek at forward Google Calendar context for health planning questions.",
    )
    _register_tool(
        ctx,
        "health_coverage",
        ...
        description="Report health source coverage and freshness for a date window or analysis pack.",
    )
    _register_tool(
        ctx,
        "health_event_query",
    ...
    _register_tool(
        ...
        description="List supported health analysis packs and their feature requirements.",
    )
    _register_tool(
        ctx,
        ...
        description="Map a broad health question to supported analysis packs and next tools.",
    )
    _register_tool(
        ctx,
        "health_analyze",
        ...
        description="Run a registered health analysis pack with coverage and provenance.",
    )
    _register_tool(
        ctx,
        ...
        description="Explain a prior health analysis run with feature and source provenance.",
    )
...
def _register_tool(ctx, name, handler, schema, description=""):
...
def _register_cli_command(ctx, name, commands_module):
    try:
